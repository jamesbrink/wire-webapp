...
z.util.DebugUtil = class DebugUtil {
  ...
  blockAllConnections() {
    const blockUsers = wire.app.repository.user.users().map(user_et => this.userRepository.block_user(user_et));
  ...
  getNotificationsFromStream(remoteUserId, remoteClientId, matchingNotifications = [], notificationIdSince) {
        ...
        : notifications.filter(notification => {
            ...
            for (const {data, from} of payload) {
              if (data && [localUserId, remoteUserId].includes(from)) {
                ...
                const incomingEvent = sender === remoteClientId && recipient === localClientId;
                const outgoingEvent = sender === localClientId && recipient === remoteClientId;
                return incomingEvent || outgoingEvent;
              }
            }
            return false;
          });
    ...
    const client_scope = remoteUserId === localUserId ? undefined : localClientId;
    return wire.app.service.notification
      .getNotifications(client_scope, notificationIdSince, 10000)
      .then(_gotNotifications);
  }

  get_objects_for_decryption_errors(sessionId, notificationId) {
    return Promise.all([
